...
def patch_head_for_export(model, dev):
    ...
    # IMPORTANT: set directly, do not guard with hasattr()
    head.export = True
    head.dynamic = False
    head.format = "onnx"
    ...
    # Pose.forward() calls self.detect(self, x) in your version
...
def main():
    ...
    dummy = torch.zeros(1, 1, IMGSZ, IMGSZ, device=dev)
    ...
    torch.onnx.export(
        ...
        dynamic_axes=None,   # static 640x640
        verbose=False,
    )
