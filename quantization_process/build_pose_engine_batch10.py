...
# For static batch engine, calibration batch should match the exported batch.
CALIB_BATCH = int(os.environ.get("CALIB_BATCH", str(BATCH_SIZE)))
...
INT8_ENGINE = OUT_DIR / f"int8_batch{BATCH_SIZE}.engine"
FP16_ENGINE = OUT_DIR / f"fp16_batch{BATCH_SIZE}.engine"
FP32_ENGINE = OUT_DIR / f"fp32_batch{BATCH_SIZE}.engine"

# avoid reusing old TRT cache from other batch settings
CALIB_CACHE = OUT_DIR / f"calib_batch{BATCH_SIZE}.cache"
...
def build_engine(out_path: Path, fp16=False, int8=False, calib_files=None):
    ...
    log(f"[TRT] input name={inp.name} shape={inp_shape} dtype={inp.dtype}")

    # This script targets a static batch=BATCH_SIZE ONNX.
    if any(d == -1 for d in inp_shape):
        raise RuntimeError(
            "This script expects a static-batch ONNX, but got dynamic input shape. "
            "Please export ONNX with dynamic_axes=None and BATCH_SIZE fixed to 3."
        )

    if inp_shape != (BATCH_SIZE, 1, IMGSZ, IMGSZ):
        raise RuntimeError(
            f"Unexpected ONNX input shape {inp_shape}; expected {(BATCH_SIZE, 1, IMGSZ, IMGSZ)}"
        )
    ...
    if int8:
        ...
        if CALIB_BATCH != BATCH_SIZE:
            raise RuntimeError(
                f"For this static batch engine, CALIB_BATCH must match BATCH_SIZE. "
                f"Got CALIB_BATCH={CALIB_BATCH}, BATCH_SIZE={BATCH_SIZE}"
            )
...
def main():
    ...
    log(f"[CFG] batch_size={BATCH_SIZE}")
    log(f"[CFG] calib_batch={CALIB_BATCH}")
    ...
    if CALIB_BATCH < 1:
        raise ValueError(f"CALIB_BATCH must be >= 1, got {CALIB_BATCH}")
